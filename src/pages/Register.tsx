...
const Register: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch("http://localhost/cloudgames/register.php", {
            ...
            if (response.ok && data?.status === "success") {
                ...
                else setError(`Registration failed${response.status ? ' (' + response.status + ')' : ''}`);
            }
        } catch (err) {
            // If server responded with a specific message that we didn't parse earlier
